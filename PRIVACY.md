...
Vowntil does not require an account and does not collect, sell, rent, track, or transmit your personal data to Nerdquad. The app has no advertising SDK, analytics SDK, or app-initiated backend connection.

Vowntil uses Apple system frameworks to perform its core features. App selections, schedules, lock history, settings, and related operational records are stored locally on your device. Screen Time usage reports are prepared and displayed through Apple's privacy-preserving Device Activity report extension.
...
- Named presets, restriction modes, schedules, lock start and end times, and app settings.
- Local lock History and enforcement events, including authorization changes and whether a scheduled or manual lock completed or was interrupted.
...
Vowntil can create a diagnostics file on your device when you explicitly request one. The file can contain app and iOS versions, authorization status, timestamps, session identifiers, selection counts, active-lock details, and recent local enforcement events.
...
Vowntil relies on iOS sandboxing, app-group access controls, code signing, and Apple's Screen Time privacy protections. No method of storage is guaranteed to be completely secure, but the app minimizes risk by avoiding accounts, advertising, analytics, and app-initiated transmission of your data.
